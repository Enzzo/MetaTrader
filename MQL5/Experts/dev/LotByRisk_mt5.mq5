...
bool Trade(){
   ...
   if(pr == 0.0){
      ...
      //3)
      if(sl == 0.0){
         if(tp > Ask) return trade.Buy(AutoLot(risk, pts), Symbol(), sl, tp, SLIPPAGE, cmnt);
         if(tp < Bid) return trade.Sell(AutoLot(risk, pts), Symbol(), sl, tp, SLIPPAGE, cmnt);
      ...
      //4)
      if(tp == 0.0){
         if(sl < Bid) return trade.Buy(AutoLot(risk, pts), Symbol(), sl, tp, SLIPPAGE, cmnt);
         if(sl > Ask) return trade.Sell(AutoLot(risk, pts), Symbol(), sl, tp, SLIPPAGE, cmnt);
      ...
      if(tp > Ask && sl < Bid) return trade.Buy(AutoLot(risk, pts), Symbol(), sl, tp, SLIPPAGE, cmnt);
      if(tp < Bid && sl > Ask) return trade.Sell(AutoLot(risk, pts), Symbol(), sl, tp, SLIPPAGE, cmnt);
      return Wrong("7 E");
   }
   //(5, 6, 8)
   else{
      ...
      if(sl == 0.0){
         if(tp > pr){
            if(pr > Ask)return trade.BuyStop(AutoLot(risk, pts), pr,Symbol(),  sl, tp, 0);
            if(pr < Ask)return trade.BuyLimit(AutoLot(risk, pts), pr,Symbol(), sl, tp, 0);
            
            if(pr == Ask)return trade.Buy(AutoLot(risk, pts),  Symbol(),sl, tp, SLIPPAGE, cmnt);
         }
         if(tp < pr){
