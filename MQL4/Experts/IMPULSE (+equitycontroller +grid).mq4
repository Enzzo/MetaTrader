...
input double      prof        = 1.0;   //Профит выхода из сетки
input double      k           = 5.0;   //Множитель шага сетки
input int         grid        = 20;    //Количество ордеров в сетке
...
short Signal(ushort j){
   ...
   //Против импульса
   switch(j){
      case 0:if(level > NormalizeDouble(Ask + impulse*mtp*Point(), Digits()) && level != 0.0 && (CheckMA() == 0 || CheckMA() == 1)){
            level = 0.0;
            return 0;
         }
      break;
      
      case 1:if(level < NormalizeDouble(Bid - impulse*mtp*Point(), Digits()) && level != 0.0 && (CheckMA() == 0 || CheckMA() == 2)){
            level = 0.0;
            return 1;
         }
      break;
   ...
   switch(s){
      case 0:
         if(Total(OP_BUYSTOP) == 0){
            trade.BuyStop(Symbol(), AutoLot(volume), NormalizeDouble(Ask + stopOffset*mtp*Point(), Digits()), SL, TP, botName);
            trade.Sell(Symbol(), AutoLot(volume), SL2, TP2, botName);
         }
         break;
      ...
      case 1:
         if(Total(OP_SELLSTOP) == 0 && (type == stops || type == both)){
            trade.SellStop(Symbol(), AutoLot(volume), NormalizeDouble(Bid - stopOffset*mtp*Point(), Digits()), SL, TP, botName);
            trade.Buy(Symbol(), AutoLot(volume), SL2, TP2, botName);
         }
         break;
...
void BuildGrid(ushort j){
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--){
      ...
      if(OrderSymbol() == Symbol() && OrderMagicNumber() == magic && OrderType() == j){
         if(OrderType() == OP_BUY){
            if(Ask < (OrderOpenPrice() - avrCnd*k))trade.Buy(Symbol(), OrderLots(), 0, 0, botName);
            return;         
         }
         if(OrderType() == OP_SELL){
            if(Bid > (OrderOpenPrice() + avrCnd*k))trade.Sell(Symbol(), OrderLots(), 0, 0, botName);
            return;
         }
      }
   }
...
short CheckMA(){
   ...
   switch(pos){
      ...
      case 1:           //Если против направления, то 1 - для покупки, 2 - для продажи
         ...
         break;
      case 2:           //Если по направлению, то 1 - для покупки, 2 - для продажи
         if(rates[1].high < iMA(Symbol(), PERIOD_CURRENT, MAperiod, MAshift, MODE_EMA, PRICE_CLOSE, 1)){
            return 2;   
            }
         if(rates[1].low > iMA(Symbol(), PERIOD_CURRENT, MAperiod, MAshift, MODE_EMA, PRICE_CLOSE, 1)){
