...
void Start(){
   ...
   RefreshRates();
   CopyRates(Symbol(), PERIOD_D1, 0, 3, D1_rates); //текущая и предыдущая для определения направления
   CopyRates(Symbol(), PERIOD_H4, 0, 3, H4_rates); //текущая и предыдущая для определения направления
   CopyRates(Symbol(), PERIOD_H1, 0, 3, H1_rates); //две предыдущих для расчета свечи HeikenAshi
   
   if(ArraySize(D1_rates) < 3 || ArraySize(H4_rates) < 3 || ArraySize(H1_rates) < 3)
      return;
   ...
   double D1_MA = iMA(Symbol(), PERIOD_D1, /*D1_MA_period*/24, 0, MODE_EMA, PRICE_CLOSE, 1);
   double H4_MA = iMA(Symbol(), PERIOD_H4, /*H4_MA_period*/24, 0, MODE_EMA, PRICE_CLOSE, 1);
   
   D1_Open[0]   = (D1_rates[1].open  +  D1_rates[1].close)/2;
   D1_Close[0]  = (D1_rates[0].open  +  D1_rates[0].close  +  D1_rates[0].high + D1_rates[0].low)/4;
   D1_Open[1]   = (D1_rates[2].open  +  D1_rates[2].close)/2;
   D1_Close[1]  = (D1_rates[1].open  +  D1_rates[1].close  +  D1_rates[1].high + D1_rates[1].low)/4;
   
   H4_Open[0]   = (H4_rates[1].open  +  H4_rates[1].close)/2;
   H4_Close[0]  = (H4_rates[0].open  +  H4_rates[0].close  +  H4_rates[0].high + H4_rates[0].low)/4;
   H4_Open[1]   = (H4_rates[2].open  +  H4_rates[2].close)/2;
   H4_Close[1]  = (H4_rates[1].open  +  H4_rates[1].close  +  H4_rates[1].high + H4_rates[1].low)/4;
   
   H1_Open[0]   = (H1_rates[1].open  +  H1_rates[1].close)/2;
   H1_Close[0]  = (H1_rates[0].open  +  H1_rates[0].close  +  H1_rates[0].high + H1_rates[0].low)/4;
   H1_Open[1]   = (H1_rates[2].open  +  H1_rates[2].close)/2;
   H1_Close[1]  = (H1_rates[1].open  +  H1_rates[1].close  +  H1_rates[1].high + H1_rates[1].low)/4;
   
   //BUY   
   if(D1_Open[0] < D1_Close[0] && D1_Open[1] < D1_Close[1] && D1_Close[0] > D1_MA){
      if(H4_Open[0] < H4_Close[0] && H4_Open[1] < H4_Close[1] && H4_Close[0] > D1_MA){
         if(H1_Open[0] < H1_Close[0] && H1_Open[1] < H1_Close[1]){
