...
void SetDirection(unsigned short& d){
   //TODO: ограничивать срабатывание на 1 день
   MqlRates d1[];
   //MqlRates h1[];
   ArraySetAsSeries(d1, true);
   //ArraySetAsSeries(h1, true);

   CopyRates(Symbol(), PERIOD_D1, 1, 2, d1);
   if(d1[0].close > (d1[1].high + offset)){
      if(DRAW_ABSORP){
         //ищем индекс наивысшего значения и сохраняем значение и время в переменные h и t
         int i = iHighest(Symbol(), PERIOD_H1, MODE_HIGH, 23, 1);
         double h = iHigh(Symbol(), PERIOD_H1, i);
         datetime t = iTime(Symbol(), PERIOD_H1, i);
         //обновляем индекс и ищем низший low
         i = iLowest(Symbol(), PERIOD_H1, MODE_LOW, 23, 1);
         double l = iLow(Symbol(), PERIOD_H1, i);

         RectangleCreate("up", d1[0].time, h, t, l, clrGreen);
      }
      //Переключаем d на MODE_LOWER и ориентируемся на нижние фракталы
      d = 2;
   }
   else if(d1[0].close < (d1[1].low - offset)){
      if(DRAW_ABSORP) {
         //ищем индекс низшего значения и сохраняем значение и время в переменные l и t
         int i = iLowest(Symbol(), PERIOD_H1, MODE_LOW, 23, 1);
         double l = iLow(Symbol(), PERIOD_H1, i);
         datetime t = iTime(Symbol(), PERIOD_H1, i);
         //обновляем индекс и ищем высший high
         i = iHighest(Symbol(), PERIOD_H1, MODE_HIGH, 23, 1);
         double h = iHigh(Symbol(), PERIOD_H1, i);

         RectangleCreate("dn", d1[0].time, h, t, l, clrRed);
      }
