//+------------------------------------------------------------------+
//| EA_1Click_12Pairs_OnChart_V05.mq4
//+------------------------------------------------------------------+
#property strict

//--- Custom Libraries
#include <LIB_OBJECTS.mqh>
...
int    i_BarCounter=0;
string l_Prefix,l_ECN,l_Lot,l_Slip,l_TP,l_SL,l_Gap,l_Trail,l_Bal,l_Eq,l_EqLoss,l_DayPP,l_OpenPP,l_OrdCnt;
...
//--- Chart Header
   ...
   objLabel(l_H2,CORNER_LEFT_UPPER,140,5,"Fixedsys",9,clrGold,"*STRENGTH*");
      ...
      for(aidx=0; aidx<MAXSIZE; aidx++)
        {
         ...
         if(A_Signals[aidx][0]==1)
            objLabel(A_SignalObj[aidx],CORNER_LEFT_UPPER,120,50+aidx*30,"Impact",14,clrMediumSeaGreen,"WEAK   ["+IntegerToString(A_SignalBars[aidx],2)+" bars]");
         else if(A_Signals[aidx][0]==2)
            objLabel(A_SignalObj[aidx],CORNER_LEFT_UPPER,120,50+aidx*30,"Impact",14,clrLime,"STRONG   ["+IntegerToString(A_SignalBars[aidx],2)+" bars]");
         else if(A_Signals[aidx][0]==-1)
            objLabel(A_SignalObj[aidx],CORNER_LEFT_UPPER,120,50+aidx*30,"Impact",14,clrOrangeRed,"WEAK   ["+IntegerToString(A_SignalBars[aidx],2)+" bars]");
         else if(A_Signals[aidx][0]==-2)
            objLabel(A_SignalObj[aidx],CORNER_LEFT_UPPER,120,50+aidx*30,"Impact",14,clrRed,"STRONG   ["+IntegerToString(A_SignalBars[aidx],2)+" bars]");
