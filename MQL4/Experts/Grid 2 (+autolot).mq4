...
input int      magic = 8724312;
//input double   lot   = 0.01;
input double   prof  = 2.0;      //стартовая сумма закрытия ордеров
...
void Start(){
   ...
   */
   ...
   if(price != 0.0){
      ...
      if(!OrderSend(Symbol(), cmd, sVol, price, slp, 0.0, 0.0, "Grid_2(+autolot)", magic)){
         Print(__FUNCTION__,"  ",GetLastError());
         return;
      }
   }
...
void BuildGrid(int dir = -1){
   ...
   if(price != 0){
      ResetLastError();
      if(!OrderSend(Symbol(), dir, sVol*2, price, slp, 0.0, 0.0, "Grid_2(+autolot)", magic)){
         Print(__FUNCTION__,"  ",GetLastError());
         return;
      }
   }
