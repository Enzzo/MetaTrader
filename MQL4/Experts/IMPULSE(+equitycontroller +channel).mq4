...
input int be = 0;   //Безубыток
input int tral = 0; //tral step
input double prof = 50000.0;//Профит выхода из просадки
input bool auto = true; //Автолот от риска
input double volume = 0.01; //lot
input cents risk = 1;  //риск для автолота
input bool autoclose = true;//автозакрытие по профиту
input cents prft = 1;//Профит
input cents loss = 100;//Просадка
...
short Signal(){
   static double level = 0.0; //нулевой уровень, от которого отсчитываем импульс
   
   if(level == 0.0)
      level = Bid;
   
   if(level > NormalizeDouble(Ask + impulse*mtp*Point(), Digits()) && level != 0.0 && (CheckMA() == 0 || CheckMA() == 1)){
      level = 0.0;
      return 0; 
   }    
   if(level < NormalizeDouble(Bid - impulse*mtp*Point(), Digits()) && level != 0.0 && (CheckMA() == 0 || CheckMA() == 2)){
      level = 0.0;
      return 1;
   }
   return -1;
...
void Mode1(){
   ...
   if(Total() == 0){ 
      switch(s){
         case 0:/*if(Total(OP_BUYSTOP) == 0 && (type == stops || type == both))*/trade.BuyStop(Symbol(), AutoLot(volume), NormalizeDouble(Ask + stopOffset*mtp*Point(), Digits()), SL, TP, botName);/*trade.CloseSell(); trade.DeleteType(OP_SELLSTOP);*/break;
         ...
         case 1:/*if(Total(OP_SELLSTOP) == 0 && (type == stops || type == both))*/trade.SellStop(Symbol(), AutoLot(volume), NormalizeDouble(Bid - stopOffset*mtp*Point(), Digits()), SL, TP, botName);/*trade.CloseBuy(); trade.DeleteType(OP_BUYSTOP);*/break;
...
void BuildGrid(){
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--){
      ...
      if(OrderSymbol() == Symbol() && OrderMagicNumber() == magic){
         if(OrderType() == OP_BUY){
            if(Ask < (OrderOpenPrice() - avrCnd*5))trade.Sell(Symbol(), OrderLots()*2, 0, 0, botName);
            return;         
         }
         if(OrderType() == OP_SELL){
            if(Bid > (OrderOpenPrice() + avrCnd*5))trade.Buy(Symbol(), OrderLots()*2, 0, 0, botName);
            return;
         }
      }
   }
